...
function annotatorImageSelect(options) {
  ...
  var imgselect_utils = {
    ...
    selectionSetup: function() {
      jQuery(document)
        .on('click.imgselection','.imgareaselect-outer', '.annotator-cancel', function(evt) {
          jQuery(".tmp-img-selection").remove();
        });
      // escape key exits editor, so should also clear temporary selection
      jQuery('.annotator-editor textarea').on('keydown', function(e) {
    ...
    hideAnnotatorEditorAdder: function() {
      ...
      var jQueryvisible_editor = jQuery(".annotator-editor:not(.annotator-hide)");
      if (jQueryvisible_editor.length > 0){
        jQueryvisible_editor.addClass('annotator-hide');
      }
    ...
    selectionEnd: function(img, selection) {
      ...
      jQuery(".annotator-adder+div").addClass('active-img-selection');
      ...
      var offset = jQuery(s.adder.element[0]).offset();
      if (offset){
        jQuery(".annotator-editor").css({
          top: offset.top + 50,
    ...
    selectionPosition: function(img, selection) {
      ...
      var img_offset = jQuery(img).offset();
